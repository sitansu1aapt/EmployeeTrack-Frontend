...
class MyMessagingService : FirebaseMessagingService() {
    ...
    private fun showEmergencyNotification(title: String?, body: String?) {
        Log.d("FCM", "showEmergencyNotification: title=$title body=$body")
    ...
    private fun createNotificationChannel(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            // For critical channels like emergency, recreate the channel so sound changes take effect
            if (channelId == "emergency_channel") {
                try { notificationManager.deleteNotificationChannel(channelId) } catch (_: Exception) {}
            }
            ...
            val channel = NotificationChannel(channelId, channelName, importance).apply {
                ...
                soundUri?.let {
                    ...
                    // Treat emergency channel as an alarm for louder playback
                    if (channelId == "emergency_channel") {
                        attrsBuilder.setUsage(AudioAttributes.USAGE_ALARM)
                    } else {
                        attrsBuilder.setUsage(AudioAttributes.USAGE_NOTIFICATION)
                    }
